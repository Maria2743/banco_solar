...
const createTransferencias = async ({ emisor, receptor, monto }) => {
    try {
        await pool.query("BEGIN");

        const emisorQuery = {
            text: `SELECT uid FROM usuarios WHERE nombre = $1`,
            values: [emisor]
        };
        const receptorQuery = {
            text: `SELECT uid FROM usuarios WHERE nombre = $1`,
        ...
        const emisorId = emisorResult.rows[0].uid;
        const receptorId = receptorResult.rows[0].uid;

        const saldoEmisorQuery = {
            text: `SELECT balance FROM usuarios WHERE uid = $1`,
            values: [emisorId]
        };
        ...
        const updateBalanceEmisorQuery = {
            text: `UPDATE usuarios SET balance = balance - $1 WHERE uid = $2`,
